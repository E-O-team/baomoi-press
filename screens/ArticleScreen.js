...
export default class ArticleScreen extends React.Component {
  ...
  static navigationOptions = ({navigation}) => {
    return {title: "Article", tabBarVisible: false, header: null}
  }
  ...
  getTimeCount = async () => {
    ...
    if (value !== null)
      this.setState({currentCount: Number.parseInt(value)})
  ...
  timer = async () => {
    ...
    if (this.state.currentCount >= 7200)
  ...
  fetchComment = () => {
    ...
    fetch("https://baomoi.press/wp-json/wp/v2/comments?post=" + this.state.Article.id + "&per_page=" + request_length.toString(), {cancelToken: this.cancelTokenSource.token}).then(res => res.json()).then(json => {
      this.setState({
        comments: json
      }, () => {
        if (this.state.comments.length >= request_length)
          this.fetchComment()
      })
    })
  ...
  setAds = async () => {
    //Get PopUpAds
    const interstitialList = await AsyncStorage.getItem('PopUpAds')
    this.setState({
      PopUpAds: JSON.parse(interstitialList)
    }, () => {
      const AdTypes = ["Admob", "Facebook"]
      const randomType = AdTypes[Math.floor(Math.random() * AdTypes.length)]
      this.state.PopUpAds.forEach(item => {
        if (item.acf.os == Platform.OS && item.acf.AdPosition == "Giữa ứng dụng" && item.acf.source == randomType) {
          this.setState({
            PopUpAds: [item]
          }, () => {
            if (item.acf.source == "Admob") {
              AdMobInterstitial.setAdUnitID(item.acf.unitID);
    ...
    if (string.indexOf('<ads>') == -1) {
      ...
      if (match) {
        const new_content = [
          string.slice(0, middle_position + match.index),
          ' <ads></ads> ',
          string.slice(middle_position + match.index)
        ].join('');
  ...
  ShowPopUpAd = async () => {
    const startTime = await Number.parseInt(AsyncStorage.getItem('StartingAdTime'))
    const popUpTimes = await Number.parseInt(AsyncStorage.getItem('PopUpAdShowedTimes'))
    const PopUp = this.state.PopUpAds[0]
    console.log(startTime)
    console.log(popUpTimes)

    var neededCountdownTime
    if (popUpTimes == 0)
      neededCountdownTime = PopUp.acf.countdown_to_show_first_ad
    else
      neededCountdownTime = PopUp.acf.countdown_to_show_next_ad
    console.log("needed :" + neededCountdownTime)

    if (this.state.currentCount - startTime >= neededCountdownTime) {
      console.log("show POPUP!")
      if (PopUp.acf.source == "Admob")
        AdMobInterstitial.showAdAsync()
      if (PopUp.acf.source == "Facebook")
        FacebookAds.InterstitialAdManager.showAd(PopUp.acf.unitID).then(didClick => {}).catch(error => console.log(error)) ; popUpTimes += 1
      AsyncStorage.setItem("PopUpAdShowedTimes", popUpTimes.toString())
      AsyncStorage.setItem("StartingAdTime", this.state.currentCount.toString())
    }
  ...
  render() {
    const headerSource = this.state.scrollY.interpolate({
      inputRange: [
        0, this.state.TopViewHeight - 10,
        this.state.TopViewHeight - 9
      ],
      outputRange: [
        0, 0, 1
      ],
    ...
    return (<Consumer>
      {
        ({textColor, backGround, fontSizeRatio}) => (<View style={{
          ...
          <Animated.View style={{
            ...
            <View style={{
              ...
              <Icon name='chevron-left' size={35} color='#696969' onPress={() => {
                  this.ShowPopUpAd()
                  this.props.navigation.goBack()
                }}/>
            </View>
            <Animated.View style={{
                opacity: headerSource,
                flex: 5
              }}>
            ...
            <View style={{
              ...
              <Icon name='dots-three-vertical' type='entypo' size={25} color='#696969' style={{
                  marginRight: 5
                }} onPress={() => this.setState({settingModalVisible: true})}/>

              <Modal transparent={true} visible={this.state.settingModalVisible} onRequestClose={() => {}}>
                <TouchableOpacity style={{
                    backgroundColor: 'rgba(0,0,0,0.5)',
                    flex: 1,
                    alignItems: 'flex-end'
                  }} onPress={() => this.setState({settingModalVisible: false})}>
                  <Consumer>
                    {
                      ({
                        changeDay,
                        ...
                        <View style={{
                          ...
                          <BaomoiText style={{
                              fontSize: 16
                            }}>Giao diện</BaomoiText>
                          <Switch onValueChange={switchMode} value={nightMode}/>
                        </View>
          ...
          <ScrollView ref={(scrollView) => {
                      ...
                      (this.state.Article.format === 'video')
                        ...
                        : <View style={{
                            ...
                            <Text style={{
                                ...
                                color: textColor,
                                marginBottom: 5
                              }}>{this.state.Article.title.plaintitle}</Text>
                  ...
                  }}>
                  {
                    (this.state.Article.format === 'video')
                      ...
                      : <View style={{
                          ...
                          <HTML html={this.state.Article.excerpt.custom_excerpt} imagesMaxWidth={Dimensions.get('window').width - 20} onLinkPress={(event, href) => {
                              Linking.openURL(href)
                            }} ignoredStyles={['width', 'height', 'max-width']} staticContentMaxWidth={Dimensions.get('window').width - 20} tagsStyles={{
                              blockquote: {
                                marginLeft: 50
                              },
                              p: {
                              ...
                              fontSize: 19 * fontSizeRatio,
                              fontWeight: '500',
                              fontFamily: 'baomoi-regular',
                              color: textColor,
                              lineHeight: 23 * fontSizeRatio
                            }}/>
                          ...
                          <HTML alterChildren= { (node) => {
                                    ...
                                    } html={this.state.Article.content.plaintext} imagesMaxWidth={Dimensions.get('window').width - 20} onLinkPress={(event, href) => {
                              Linking.openURL(href)
                            }} ignoredStyles={['width', 'height', 'max-width']} staticContentMaxWidth={Dimensions.get('window').width - 20} tagsStyles={{
                              blockquote: {
                                marginLeft: 50
            ...
            <View style={{
              ...
              <TouchableOpacity style={{
                ...
                <Icon name='share-2' type='feather' color='white' size={30}/>
                <BaomoiText style={{
                    color: '#ffffff',
                    fontSize: 22,
                    marginLeft: 5
